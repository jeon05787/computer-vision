...
# 3. CNN 모델 구축
# Sequential 모델을 사용하여 신경망 층을 순차적으로 쌓아 모델을 구성
model = Sequential([
    # 합성곱 블록 1: 32개의 필터를 사용한 Conv2D 레이어와 MaxPooling2D 레이어
    # Conv2D는 이미지의 특징을 추출하는 합성곱 레이어
    Conv2D(32, (3, 3), activation='relu', padding='same', input_shape=(32, 32, 3)),
    Conv2D(32, (3, 3), activation='relu', padding='same'),
    # MaxPooling2D는 이미지 크기를 절반으로 줄이며, 계산량을 줄이고 과적합을 방지하는 데 도움
    MaxPooling2D(pool_size=(2, 2)),   # 풀링 후 이미지 크기가 32×32 → 16×16

    # 합성곱 블록 2: 64개의 필터를 사용한 Conv2D 레이어와 MaxPooling2D 레이어
    Conv2D(64, (3, 3), activation='relu', padding='same'),
    Conv2D(64, (3, 3), activation='relu', padding='same'),
    MaxPooling2D(pool_size=(2, 2)),   # 풀링 후 이미지 크기가 16×16 → 8×8

    # 완전 연결층: 이미지를 1차원 벡터로 평탄화하고, Dense 레이어를 추가
    Flatten(),  # 2D 이미지 데이터를 1D 배열로 평탄화
    Dense(512, activation='relu'),  # 512개의 유닛을 가진 Dense 레이어
    Dense(10, activation='softmax'),  # 출력 레이어, 10개의 출력 유닛 (CIFAR-10의 클래스 수)
])

# 4. 모델 컴파일: Adam 옵티마이저와 Sparse Categorical Crossentropy 손실 함수 사용
model.compile(
    optimizer='adam',  # Adam 옵티마이저는 학습률을 자동으로 조정하여 빠른 수렴을 돕는 최적화 알고리즘
    loss='sparse_categorical_crossentropy',  # 다중 클래스 분류 문제에서 사용되는 손실 함수
    metrics=['accuracy']  # 모델의 성능을 평가하기 위한 지표, 여기서는 정확도를 사용
)

# 5. 모델 훈련: 10 에폭 동안 학습
# 학습 데이터와 레이블을 모델에 제공하여 모델이 패턴을 학습하도록 함
history = model.fit(
    x_train, y_train,
    epochs=10,  # 전체 데이터셋을 10번 반복하여 학습
    batch_size=64,  # 한 번에 학습할 샘플의 개수
    validation_split=0.1,  # 10%는 검증용 데이터로 사용
...
# 7. 테스트 결과 출력
# 테스트 손실과 정확도를 출력하여 모델의 성능을 확인
print("=" * 45)
print("테스트 결과")
print(f"  테스트 손실(Loss): {test_loss:.4f}")  # 손실 값 출력
print(f"  테스트 정확도:     {test_acc * 100:.2f}%")  # 정확도 출력
print("=" * 45)
...
if os.path.exists(IMG_PATH):
    ...
    # 예측 결과 출력
    print("=" * 45)
    print(f"예측 결과: {CLASS_NAMES[pred_class]}")  # 예측된 클래스 이름 출력
